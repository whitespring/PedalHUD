...
final class RideOverlayCameraCaptureSource: NSObject, AVCaptureVideoDataOutputSampleBufferDelegate, @unchecked Sendable {
    private let session = AVCaptureSession()
    private let sessionQueue = DispatchQueue(label: "RideOverlayCameraCaptureSource.session")
    private let videoOutputQueue = DispatchQueue(label: "RideOverlayCameraCaptureSource.video")
    ...
    func startAsync() {
        sessionQueue.async { [self] in
            do {
                ...
                NSLog(
                    "RideOverlayCameraExtension failed to start webcam capture asynchronously: %@",
    ...
    private func configureSession() throws {
        ...
        if session.canAddInput(input) {
            ...
            throw NSError(
                domain: "RideOverlayCameraCaptureSource",
                code: 2,
                userInfo: [
    ...
    private func selectCaptureDevice() throws -> AVCaptureDevice {
        ...
        let devices = discoverySession.devices.filter { device in
            !device.localizedName.localizedCaseInsensitiveContains("Ride Overlay")
        }
